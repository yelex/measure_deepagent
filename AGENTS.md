...
- [ ] Зафиксировать модель после теста качества цитирования (Claude / GigaChat / GLM) —
  **решение пользователя (2026-08-01): протестировать все три**, не
  сразу фиксировать Claude по умолчанию. Для GigaChat/GLM понадобятся
  отдельные API-ключи от пользователя (Claude уже доступен через
  Claude Code, без дополнительной настройки) — актуально, когда B001
  дойдёт до реального LLM-экстрактора вместо текущих regex-эвристик в
