...
Три роли, три промпт-файла, ротация в `ralph_loop.sh` (см. ниже).
Разделение введено после инцидента `cbb644c` (Coder сам себе
подтвердил успех, смягчив scorer) — теперь ни один агент не является
одновременно исполнителем и судьёй собственной работы.

- **Coder** (`RALPH_PROMPT_CODER.md`) — берёт одну todo-задачу, правит
  код, коммитит как `wip:`, передаёт результат через
  `data/output/ralph_handoff.json`. Не принимает финальное решение об
  успехе.
- **Tester** (`RALPH_PROMPT_TESTER.md`) — независимо перепроверяет
  последний `wip:`-коммит: целостность защищённых файлов (automatic
  reject при их изменении, без исключений), перезапуск eval с нуля,
  регрессионные проверки на прошлых багах (L001, L006, L008). Выносит
  вердикт confirm/revert, только Tester пишет финальную запись в
  `tuning_log.jsonl` и меняет статус задачи на `done`/`reverted`.
- **Analyst** (`RALPH_PROMPT_ANALYST.md`) — запускается раз в
  `ANALYST_EVERY_N` циклов (не каждую итерацию). Не трогает код.
  Смотрит на тренды по `tuning_log.jsonl` целиком, делает root-cause
  разборы стагнирующих метрик, перестраивает приоритеты в
  `IMPROVEMENT_BACKLOG.md`, решает судьбу тяжёлых экспериментов
  (например L007).
